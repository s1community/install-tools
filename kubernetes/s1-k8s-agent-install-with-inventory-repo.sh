...
#
# Pre-Requisite: MUST have an s1.config file in the same directory path as this script
# 
# Usage:  sudo ./s1-k8s-agent-install-with-inventory-repo.sh 
# 
# Version:  2025.04.19
#
...
if [ -z $S1_AGENT_TAG ];then
    echo ""
    read -p "Please enter the SentinelOne Agent Version to install (ie: 24.3.3-ga): " S1_AGENT_TAG
fi
...
HELM_RELEASE_VERSION=$(echo $S1_AGENT_TAG | cut -d "-" -f1) # ie: 24.3.2
...
helm upgrade --install ${HELM_RELEASE_NAME} --namespace=${S1_NAMESPACE} --version ${HELM_RELEASE_VERSION} \
    ...
    --set configuration.cluster.uid=${CLUSTER_UID} \
